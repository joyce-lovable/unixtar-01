...
const MoldDataTable = () => {
  ...
  const fetchGroups = async () => {
    const { data, error } = await supabase
      .from('mold_groups')
      .select('group_id, part_name, downloaded')
      .order('group_id');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const gid of groupIdsToCheck) {
        ...
        if (count === 0) {
          await supabase.from('mold_groups').delete().eq('group_id', gid);
        }
      }
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-blue-50 dark:bg-blue-950/30">
              ...
              <TableHead className="min-w-[200px] whitespace-nowrap">品名</TableHead>
              <TableHead className="min-w-[150px] whitespace-nowrap">模號</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              ...
              records.map((record, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="font-medium whitespace-nowrap">{record.part_name}</TableCell>
                    <TableCell className="font-mono text-sm whitespace-nowrap">{record.mold_number}</TableCell>
                  </TableRow>
                );
              })
            )}
